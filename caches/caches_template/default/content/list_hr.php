    ...
    <div class="row">
        <div class="span3 sidebar">
            <div class="sidebarMenu boxModel">
                ...
                <ul>
                    <li <?php if(!$catid) { ?> class="active" <?php } ?>><a href="#"><b>&#8594;</b>全部</a></li>
                    <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=ffdd9d88b54f618e1c0c422127f12178&action=category&catid=%24CATEGORYS%5B%24parentid%5D%5Bcatid%5D&num=25&siteid=%24siteid&order=listorder+ASC\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'category')) {$data = $content_tag->category(array('catid'=>$CATEGORYS[$parentid][catid],'siteid'=>$siteid,'order'=>'listorder ASC','limit'=>'25',));}?>
                    <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
                    <li <?php if($r[catid]=="$catid") { ?> class="active"<?php } ?>><a href="<?php echo $r['url'];?>"><b>&#8594;</b><?php echo $r['catname'];?></a></li>
                    <?php $n++;}unset($n); ?>
                    <?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
                </ul>
            </div>
        ...
        <div class="span9 blogItems">
            <ul class="breadcrumb blogBreadcrumb">
                ...
                <li class="active"><a href="<?php echo $url;?>"><?php echo $CATEGORYS[$parentid]['catname'];?></a></li>
            </ul>
            <hr>
            <div class="row">
                <div class="span9">
            ...
            <!-- /.row [galleryNav]-->
            <hr>
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=0952a62279fdcf79e2722f39f18b6848&action=lists&catid=%24catid&num=3&siteid=%24siteid&order=listorder+ASC&page=%24page\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$pagesize = 3;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$content_total = $content_tag->count(array('catid'=>$catid,'siteid'=>$siteid,'order'=>'listorder ASC','limit'=>$offset.",".$pagesize,'action'=>'lists',));$pages_new = pages_new($content_total, $page, $pagesize, $urlrule);$pages = pages($content_total, $page, $pagesize, $urlrule);$data = $content_tag->lists(array('catid'=>$catid,'siteid'=>$siteid,'order'=>'listorder ASC','limit'=>$offset.",".$pagesize,'action'=>'lists',));}?>
            <ul class="portfolioItems isotope" style="margin: 0px; position: relative; overflow: hidden; height: 541px;">
                <?php $n=1;if(is_array($data)) foreach($data AS $v) { ?>
