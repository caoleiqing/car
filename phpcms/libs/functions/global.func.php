...
function pages_new($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		$multipage .= '';
		if($curr_page>0) {
			$multipage .= ' <ul class="pagination pull-right"><li><a href="'.pageurl($urlrule, $curr_page-1, $array).'" class="active">«</a></li>';
			if($curr_page==1) {
				$multipage .= ' <li><a data-index="0" href="#">1</a></li>';
			} elseif($curr_page>6 && $more) {
		...
		for($i = $from; $i <= $to; $i++) {
			if($i != $curr_page) {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $i, $array).'">'.$i.'</a></li></ul>';
			} else {
				$multipage .= ' <li><span>'.$i.'</span></li></ul>';
			}
		}
		if($curr_page<$pages) {
			if($curr_page<$pages-5 && $more) {
				$multipage .= ' ..<li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="active">»</a></li></ul>';
			} else {
				$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a></li> <li><a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="active">»</a></li></ul>';
			}
		} elseif($curr_page==$pages) {
			$multipage .= ' <li><span>'.$pages.'</span> <a href="'.pageurl($urlrule, $curr_page, $array).'">»</a></li></ul>';
		} else {
			$multipage .= ' <li><a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="active">»</a></li></ul>';
		}
	}
	return $multipage;
}
